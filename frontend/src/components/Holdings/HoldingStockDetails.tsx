...
import {
  fetchHoldings,
  fetchHoldingStockDetails,
  fetchHoldingCustomLines,
  createHoldingCustomLine,
  updateHoldingCustomLine,
  deleteHoldingCustomLine,
} from '../../services/api'
import type {
  HoldingLineCreate,
  HoldingLineUpdate,
...
// Reuse components from Watchlist (they are generic enough)
import InteractiveChart from '../Watchlist/InteractiveChart'
import TechnicalIndicatorSummary from '../Watchlist/TechnicalIndicatorSummary'
import StockNewsSection from '../Watchlist/StockNewsSection'
import CustomLinesList from '../Watchlist/CustomLinesList'

// Map HoldingLineResponse to WatchlistLineResponse for component compatibility
const mapToWatchlistLine = (line: HoldingLineResponse): WatchlistLineResponse => ({
  ...line,
  watchlist_item_id: line.holding_item_id,
})

// Map chart data to include watchlist-compatible custom_lines
const mapChartData = (data: any, lines: HoldingLineResponse[]): ChartDataResponse => ({
  ...data,
  custom_lines: lines.map(mapToWatchlistLine),
})

export default function HoldingStockDetails() {
  ...
  // Get holdings to find item ID for the ticker
  const { data: holdings } = useQuery({
    queryKey: ['holdings'],
    queryFn: () => fetchHoldings(),
  })

  const holdingItem = holdings?.items.find(
  ...
  const {
    ...
    queryKey: ['holdingStockDetails', holdingItem?.id],
    queryFn: () => fetchHoldingStockDetails(holdingItem!.id),
    enabled: !!holdingItem?.id,
  })

  // Fetch custom lines separately for easier refresh
  const { data: customLines, refetch: refetchLines } = useQuery({
    queryKey: ['holdingCustomLines', holdingItem?.id],
    queryFn: () => fetchHoldingCustomLines(holdingItem!.id),
    enabled: !!holdingItem?.id,
  })
  ...
  const handleAddLine = useCallback(
    async (priceLevel: number) => {
      ...
      try {
        const lineData: HoldingLineCreate = {
        ...
        await createHoldingCustomLine(holdingItem.id, lineData)
        await refetchLines()
    ...
    [holdingItem?.id, newLineType, refetchLines, refetchDetails]
  )

  // Handle updating a line (e.g., moving it)
  const handleUpdateLine = useCallback(
    async (lineId: number, updates: HoldingLineUpdate) => {
      if (!holdingItem?.id) return

      try {
        await updateHoldingCustomLine(holdingItem.id, lineId, updates)
        await refetchLines()
        await refetchDetails()
        setLineError(null)
      } catch (err: any) {
    ...
    [holdingItem?.id, refetchLines, refetchDetails]
  )
  ...
  const handleDeleteLine = useCallback(
    async (lineId: number) => {
      ...
      try {
        await deleteHoldingCustomLine(holdingItem.id, lineId)
        await refetchLines()
        await refetchDetails()
        setLineError(null)
      } catch (err: any) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <button onClick={() => navigate('/holdings')} className="btn-primary mt-4">
            Back to Holdings
          </button>
        </div>
      </div>
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-4">"{ticker}" is not in your holdings</p>
        ...
        {stockDetails?.item && (
          <div className="text-right">
            ...
            <div className="text-sm text-gray-500 mt-2">
              {stockDetails.item.quantity} shares @ {formatCurrency(stockDetails.item.cost_basis)}
            </div>
            {stockDetails.item.profit_loss !== undefined && (
              <div className={`text-sm font-medium ${
                stockDetails.item.profit_loss >= 0 ? 'text-success-600' : 'text-danger-600'
              }`}>
                P&L: {stockDetails.item.profit_loss >= 0 ? '+' : ''}{formatCurrency(stockDetails.item.profit_loss)}
                {stockDetails.item.profit_loss_pct !== undefined && (
                  <span className="ml-1">
                    ({stockDetails.item.profit_loss_pct >= 0 ? '+' : ''}{stockDetails.item.profit_loss_pct.toFixed(2)}%)
                  </span>
                )}
              </div>
            )}
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="card card-body">
            ...
            <InteractiveChart
              chartData={mapChartData(stockDetails.chart_data, customLines || stockDetails.custom_lines)}
              customLines={(customLines || stockDetails.custom_lines).map(mapToWatchlistLine)}
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            {/* Custom Lines */}
            <CustomLinesList
              lines={(customLines || stockDetails.custom_lines).map(mapToWatchlistLine)}
