...
from backend.app.db.models import HoldingItem, HoldingLine, HoldingTriggeredAlert
...
from backend.app.schemas.holdings import (
    HoldingItemCreate,
    HoldingItemUpdate,
    HoldingItemInfo,
    HoldingResponse,
...
async def _fetch_news_data(ticker: str, limit: int = 50) -> dict[str, Any]:
    ...
    try:
        ...
        # Use 48-hour window for "recent" news
        now = datetime.utcnow()
...
def _calculate_portfolio_metrics(
    ...
    """Calculate portfolio metrics for a holding."""
    market_value = None
    total_cost = None
    profit_loss = None
    ...
    if cost_basis is not None:
        total_cost = round(quantity * cost_basis, 2)

        if market_value is not None and total_cost > 0:
            profit_loss = round(market_value - total_cost, 2)
            profit_loss_pct = round((profit_loss / total_cost) * 100, 2)
...
@router.get("/", response_model=HoldingResponse)
async def get_holdings(
    ...
    """Get all holdings with current prices and portfolio metrics."""
    # Get all active holdings from DB
    stmt = select(HoldingItem).where(HoldingItem.is_active == True).order_by(HoldingItem.sort_order.asc())
    result = await db.execute(stmt)
    items = result.scalars().all()

    if not items:
        return HoldingResponse(total=0, items=[])
    ...
    # Fetch news data only if requested
    if include_news:
        news_results = []
    ...
    triggered_stmt = (
        select(HoldingTriggeredAlert.line_id)
        .join(HoldingLine)
        .where(
            and_(
                HoldingTriggeredAlert.triggered_at >= today_start,
                HoldingTriggeredAlert.dismissed == False,
            )
        )
    )
    ...
    if triggered_line_ids:
        lines_stmt = select(HoldingLine.holding_item_id).where(
            HoldingLine.id.in_(triggered_line_ids)
        )
    ...
    for i, item in enumerate(items):
        ...
        portfolio_metrics = _calculate_portfolio_metrics(quantity, cost_basis, current_price)

        if portfolio_metrics["market_value"]:
            total_market_value += portfolio_metrics["market_value"]
        if portfolio_metrics["total_cost"]:
        ...
        response_items.append(
            HoldingItemInfo(
                ...
                market_value=portfolio_metrics["market_value"],
                total_cost=portfolio_metrics["total_cost"],
                profit_loss=portfolio_metrics["profit_loss"],
                profit_loss_pct=portfolio_metrics["profit_loss_pct"],
    ...
    total_profit_loss = round(total_market_value - total_cost, 2) if total_cost > 0 else None
    total_profit_loss_pct = round((total_profit_loss / total_cost) * 100, 2) if total_cost > 0 and total_profit_loss is not None else None
...
@router.post("/", response_model=HoldingItemInfo)
async def add_to_holdings(
    request: HoldingItemCreate,
    db: AsyncSession = Depends(get_db),
) -> HoldingItemInfo:
    ...
    existing_stmt = select(HoldingItem).where(HoldingItem.ticker == ticker)
    existing_result = await db.execute(existing_stmt)
    existing = existing_result.scalar_one_or_none()

    if existing:
        if existing.is_active:
            raise HTTPException(status_code=400, detail=f"{ticker} is already in your holdings")
        else:
            # Reactivate inactive item
            from sqlalchemy import func as sql_func
            max_order_stmt = select(sql_func.max(HoldingItem.sort_order)).where(HoldingItem.is_active == True)
            ...
            existing.cost_basis = Decimal(str(request.cost_basis)) if request.cost_basis else None
        ...
        item = HoldingItem(
            ...
            cost_basis=Decimal(str(request.cost_basis)) if request.cost_basis else None,
            is_active=True,
...
@router.put("/{item_id}", response_model=HoldingItemInfo)
async def update_holding(
    item_id: int,
    request: HoldingItemUpdate,
    db: AsyncSession = Depends(get_db),
) -> HoldingItemInfo:
    """Update a holding's quantity or cost basis."""
    stmt = select(HoldingItem).where(HoldingItem.id == item_id, HoldingItem.is_active == True)
    result = await db.execute(stmt)
    ...
    if request.quantity is not None:
        item.quantity = Decimal(str(request.quantity))
    if request.cost_basis is not None:
        item.cost_basis = Decimal(str(request.cost_basis))

    await db.commit()
    await db.refresh(item)

    # Fetch current price
    price_data = await _fetch_stock_price(item.ticker)
...
async def remove_from_holdings(
    item_id: int,
    db: AsyncSession = Depends(get_db),
) -> dict[str, str]:
    """Remove a stock from holdings."""
    stmt = select(HoldingItem).where(HoldingItem.id == item_id)
    result = await db.execute(stmt)
    item = result.scalar_one_or_none()
    ...
    return {"message": f"{item.ticker} removed from holdings"}


@router.put("/order", response_model=dict)
async def update_holdings_order(
    request: HoldingOrderUpdate,
    db: AsyncSession = Depends(get_db),
) -> dict[str, str]:
    """Update the sort order of holdings."""
    item_ids = [item.id for item in request.items]
    stmt = select(HoldingItem).where(
        HoldingItem.id.in_(item_ids),
        HoldingItem.is_active == True,
    )
    result = await db.execute(stmt)
    ...
    if missing_ids:
        raise HTTPException(
            status_code=404,
            detail=f"Holdings not found: {missing_ids}"
        )
    ...
    logger.info("Updated holdings order", item_count=len(request.items))

    return {"message": f"Updated order for {len(request.items)} items"}


@router.get("/{item_id}/news", response_model=HoldingNewsResponse)
async def get_stock_news(
    ...
    """Get news articles for a holding."""
    stmt = select(HoldingItem).where(HoldingItem.id == item_id)
    result = await db.execute(stmt)
    item = result.scalar_one_or_none()
...
@router.get("/{item_id}/details", response_model=HoldingStockDetailsResponse)
async def get_stock_details(
    item_id: int,
    db: AsyncSession = Depends(get_db),
) -> HoldingStockDetailsResponse:
    """Get comprehensive stock details for a holding."""
    stmt = (
        select(HoldingItem)
        .where(HoldingItem.id == item_id, HoldingItem.is_active == True)
        .options(selectinload(HoldingItem.lines))
    )
    result = await db.execute(stmt)
    item = result.scalar_one_or_none()
    ...
    custom_lines = [
        HoldingLineResponse(
            id=line.id,
            holding_item_id=line.holding_item_id,
    ...
    if alert_line_ids:
        alerts_stmt = (
            select(HoldingTriggeredAlert)
            .where(
                and_(
                    HoldingTriggeredAlert.line_id.in_(alert_line_ids),
                    HoldingTriggeredAlert.triggered_at >= today_start,
                )
            )
            .order_by(HoldingTriggeredAlert.triggered_at.desc())
        )
    ...
    return HoldingStockDetailsResponse(
        item=item_info,
...
async def _fetch_news_for_details(ticker: str) -> list[NewsItem]:
    """Fetch news for stock details page."""
    try:
        client = await get_alpha_vantage_client()
...
async def get_technical_summary_endpoint(
    ...
    """Get technical analysis summary for a holding."""
    stmt = select(HoldingItem).where(HoldingItem.id == item_id, HoldingItem.is_active == True)
    result = await db.execute(stmt)
    item = result.scalar_one_or_none()
...
@router.get("/{item_id}/lines", response_model=list[HoldingLineResponse])
async def get_custom_lines(
    item_id: int,
    db: AsyncSession = Depends(get_db),
) -> list[HoldingLineResponse]:
    """Get all custom lines for a holding."""
    item_stmt = select(HoldingItem).where(HoldingItem.id == item_id, HoldingItem.is_active == True)
    ...
    stmt = select(HoldingLine).where(
        HoldingLine.holding_item_id == item_id,
        HoldingLine.is_active == True,
    ).order_by(HoldingLine.created_at.desc())
    result = await db.execute(stmt)
...
@router.post("/{item_id}/lines", response_model=HoldingLineResponse)
async def create_custom_line(
    item_id: int,
    request: HoldingLineCreate,
    db: AsyncSession = Depends(get_db),
) -> HoldingLineResponse:
    """Create a new custom line for a holding."""
    item_stmt = select(HoldingItem).where(HoldingItem.id == item_id, HoldingItem.is_active == True)
    item_result = await db.execute(item_stmt)
    item = item_result.scalar_one_or_none()
    ...
    color = request.color
    if not color:
        color = "#f97316" if request.line_type == "alert" else "#8b5cf6"

    line = HoldingLine(
        holding_item_id=item_id,
...
@router.put("/{item_id}/lines/{line_id}", response_model=HoldingLineResponse)
async def update_custom_line(
    item_id: int,
    line_id: int,
    request: HoldingLineUpdate,
    db: AsyncSession = Depends(get_db),
) -> HoldingLineResponse:
    """Update a custom line."""
    stmt = select(HoldingLine).where(
        HoldingLine.id == line_id,
        HoldingLine.holding_item_id == item_id,
    )
    result = await db.execute(stmt)
...
async def get_triggered_alerts(
    ...
    alert_responses = []
    for alert in alerts:
        line_stmt = select(HoldingLine).where(HoldingLine.id == alert.line_id)
        line_result = await db.execute(line_stmt)
        line = line_result.scalar_one_or_none()

        if line:
            item_stmt = select(HoldingItem).where(HoldingItem.id == line.holding_item_id)
            item_result = await db.execute(item_stmt)
            item = item_result.scalar_one_or_none()
...
async def dismiss_alert(
    ...
    stmt = select(HoldingTriggeredAlert).where(HoldingTriggeredAlert.id == alert_id)
    result = await db.execute(stmt)
